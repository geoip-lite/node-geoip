...
var ifile = path.join(__dirname, '/../data/geoip-city.dat');
...
lookup4 = function(ip) {
	var fline=0, floor=lastip, cline=lastline, ceil=firstip, line, cc, region, city, i;
	...
	do {
		...
		if(floor <= ip && ceil >= ip) {
			...
			region = buff.toString('utf8', line*recsz+10, line*recsz+12);
			city = buff.toString('utf8', line*recsz+12, line*recsz+32).replace(/\u0000.*/, '');
			return { range: [floor, ceil], country: cc, region: region, city: city };
		}
		else if(fline == cline) {
			return null;
		}
	...
	lookup6 = function(ip) {
		...
		do {
			...
			if(cmp6(floor, ip) <= 0 && cmp6(ceil, ip) >= 0) {
				...
				return { range: [floor, ceil], country: cc, region: "", city: "" };	// We do not currently have region/city info for IPv6
			}
			else if(fline == cline) {
				return null;
			}
