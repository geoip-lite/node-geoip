...
dotenv.config({ path: path.resolve(process.cwd(), '.geoiprc') });
	...
	do {
		...
		if (floor <= ip && ceil >= ip) {
			...
			if (recordSize === RECORD_SIZE) {
				...
				if (!process.env.GEOIP_EXCLUDE_CITY_DATA) {
...
function preload(callback) {
	...
	if (typeof arguments[0] === 'function') {
		async.series([
			function (cb) {
				async.series(!process.env.GEOIP_EXCLUDE_CITY_DATA && [
					function (cb2) {
