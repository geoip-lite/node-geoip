...
function reloadData() {
	async.series([
		function (cb) { preload(cb); },
		function (cb) { preload6(cb); }
	]);
}
...
function automaticUpdates() {
	...
	if (now.getUTCDate() <=7 && now.getUTCDay() == 3) {
		...
		var pathScript = path.join(dirModule, 'scripts', 'getdb.js');
		var getdb = fork(pathScript, { cwd: dirModule });
		getdb.on('exit', function (code) {
			reloadData();
		});
	}
	scheduleTimer = setTimeout(
		automaticUpdates, utils.nextCheck() +
		config.spreadLoad.toMaxMindServer + config.spreadLoad.gapAfterMaxMind +
		utils.random(config.spreadLoad.toIntermediateServer)
	);
}
...
module.exports = {
	...
	// Start automatic updates from intermediate server
	// 
	startAutomaticUpdates: function () {
		automaticUpdates();
	},

	// Stop automatic updates from intermediate server
	// 
	stopAutomaticUpdates: function () {
