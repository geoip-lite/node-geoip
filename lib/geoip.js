...
(function() {
var ifile, ifd, sz, recsz, buff, lrecsz, lbuff, rbuff = [];
...
try {
    ...
    fs.readFileSync(path.join(geodatadir, 'maxmind-region-codes.csv')).toString().split("\n").forEach(function (rbuffItem) {
        var rbuffPart = iconv.convert(rbuffItem).toString().split(',');

        if (rbuffPart[0] !== undefined) {
            rbuff[rbuffPart[0] + rbuffPart[1]] = (rbuffPart[2] + '').replace(/^"/, '').replace(/"$/, '');
        }
    });
}
catch(err) {
...
lookup4 = function(ip) {
	var fline=0, floor=lastip, cline=lastline, ceil=firstip, line, locId, cc, rg, rdata, rgn, city, lat, lon, i;
