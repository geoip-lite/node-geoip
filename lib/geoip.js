...
function lookup6(ip) {
	var buffer = cache6.mainBuffer6;
	var recordSize = cache6.recordSize6;
	...
	cache6.lastIP6 = readip(cache6.lastLine6, 1);
	cache6.firstIP6 = readip(0, 0);

	var fline = 0;
	var floor = cache6.lastIP6;
	var cline = cache6.lastLine6;
	var ceil = cache6.firstIP6;
	var line;

	if (utils.cmp6(ip, cache6.lastIP6) > 0 || utils.cmp6(ip, cache6.firstIP6) < 0) {
		return null;
	}
...
function preload6(callback) {
	...
	if (typeof arguments[0] === 'function') {
		async.series([
					...
					if (err) {
						...
						fs.open(dataFiles.country6, 'r', function (err, file) {
							if (err) {
								...
								fs.fstat(datFile, function (err, stats) {
									datSize = stats.size;
									asyncCache.recordSize6 = RECORD_SIZE6;
			...
			function () {
				asyncCache.mainBuffer6 = new Buffer(datSize);
				
				async.series([
					function (cb2) {
						fs.read(datFile, asyncCache.mainBuffer6, 0, datSize, 0, cb2);
					},
					function (cb2) {
						fs.close(datFile, cb2);
					}
					...
					if (err) {
						...
						asyncCache.lastLine6 = (datSize / asyncCache.recordSize6) - 1;
						cache6 = asyncCache;
					}
					callback(err);
				});
		...
		try {
			...
			datSize = fs.fstatSync(datFile).size;
			cache6.recordSize6 = RECORD_SIZE6;
		}

		cache6.mainBuffer6 = new Buffer(datSize);
		fs.readSync(datFile, cache6.mainBuffer6, 0, datSize, 0);

		fs.closeSync(datFile);

		cache6.lastLine6 = (datSize / cache6.recordSize6) - 1;
	}
}
