...
utils.aton6 = function(a) {
	...
	if (l < 7) {
		a.length = 8;
		for (i = l; i >= 0 && a[i] !== ''; i--) a[7-l+i] = a[i];
	}

	for (i = 0; i < 8; i++) {
		if (!a[i]) a[i]=0;
		else a[i] = parseInt(a[i], 16);
	}
...
// Generate interval between now and next UTC day begin
//
utils.nextCheck = function() {
	var now = new Date();
	return new Date(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), 0, 0, 0, 0) - now + dayMilliseconds
}
